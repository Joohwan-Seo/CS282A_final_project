...
class Transform(nn.Module):
    ...
    def forward(self, data_in):
        ...
        # for segmentation
        n = data_in.size()[2]
        x_rep = nn.Flatten(1)(x).repeat(n,1,1).transpose(0,2).transpose(0,1)

        out.append(x_rep)
        
        return out, x, mtx_in, mtx_feature
...
class PointNetClassification(nn.Module):
    def __init__(self, class_num = 10, dropout_prob = 0.3, use_tnet = 'yes_t'):
        ...
        self.transform = Transform(self.use_tnet)
        self.fc1 = nn.Linear(1024, 512)
        self.fc2 = nn.Linear(512, 256)
    ...
    def forward(self, data_in):
        _, x, mtx_in, mtx_feature = self.transform(data_in)
...
class PointNetSegmentation(nn.Module):
    def __init__(self, classes = 4, use_tnet = 'yes_t'):
        ...
        self.transform = Transform(self.use_tnet)
        self.fc1 = nn.Conv1d(1088, 512, 1) # 1088 = 64 + 1024
    ...
    def forward(self, data_in):
        out, _, mtx_in, mtx_feature = self.transform(data_in)
        x = torch.cat(out, 1)
        x = F.relu(self.bn1(self.fc1(x)))
